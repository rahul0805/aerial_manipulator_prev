...
plt.axis([0, err_asmc_time[-1], -8, 15])
...
plt.xlabel('time (s)')
plt.axis([0, err_asmc_time[-1], -6, 2])
plt.legend(loc=3, prop={'size': 10}, ncol=4)

fig.tight_layout(h_pad=0.4)
plt.show()
fig.savefig('error_exp2.png')
...
plt.axis([0, err_asmc_time[-1], -90, 90])
plt.legend(loc=3, prop={'size': 10}, ncol=4)
# plt.show()
# fig.savefig('err_x.png')
...
plt.axis([0, err_asmc_time[-1], -70, 110])
plt.legend(loc=2, prop={'size': 10}, ncol=4)
# plt.show()
# fig.savefig('err_y.png')
...
plt.xlabel('time (s)')
plt.axis([0, err_asmc_time[-1], -70, 90])
plt.legend(loc=2, prop={'size': 10}, ncol=4)
...
RSME_pd_ = np.zeros(3)
# RSME_pd_[0] = np.sqrt(np.mean(err_pd_x[:len(err_asmc_time)]))
# RSME_pd_[1] = np.sqrt(np.mean(err_pd_y[:len(err_asmc_time)]))
# RSME_pd_[2] = np.sqrt(np.mean(err_pd_z[:len(err_asmc_time)]))
