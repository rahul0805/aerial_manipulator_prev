...
for i in range(int(odom_time2[-1]*100)):
	...
	if ts in odom_time:
		odom_index_pid = odom_time.index(ts)
	...
	odom_x_pid.append(odom_x[odom_index_pid])
	odom_y_pid.append(odom_y[odom_index_pid])
	odom_z_pid.append(odom_z[odom_index_pid])
...
plt.plot(list(range(len(err_x_asmc))), err_y_pid, label="PID")
...
plt.plot(list(range(len(err_x_asmc))), err_z_pid, label="PID")
