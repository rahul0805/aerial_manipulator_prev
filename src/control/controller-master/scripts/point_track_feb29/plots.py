...
falling_index = err_pd_time.index(77)
...
RSME_pd_ = np.zeros(3)
RSME_pd_[0] = np.linalg.norm(err_pd_x[:len(err_asmc_time)])
RSME_pd_[1] = np.linalg.norm(err_pd_y[:len(err_asmc_time)])
RSME_pd_[2] = np.linalg.norm(err_pd_z[:len(err_asmc_time)])
RSME_pd = np.linalg.norm(RSME_pd_)

print(RSME_pd_, RSME_pd)


# print(len(err_pd_x), len(err_pd_time))